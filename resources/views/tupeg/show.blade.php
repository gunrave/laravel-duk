        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="relative overflow-x-auto shadow-md sm:rounded-lg px-4 py-4">
                    ...
                    <form method="POST" action="{{ route('tunkin-import') }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="mt-4">
                            <div class="custom-file text-left inline-flex">
                                ...
                                <input type="hidden" name="tupeg" value="{{ $tunkin->id }}">
                            </div>
                        </div>
                ...
                <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                        @forelse ($tupegs as $tupeg)
                            {{-- {{ dd($tupeg) }} --}}
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                <td class="px-6 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap">
                                    {{ $tupeg->pegawais->nama }}
                                    <p>{{ $tupeg->pegawais->nik }} </p>
                                    @foreach ($tupeg->pegawais->pangkats as $pangkat)
                                        <p>{{ $pangkat->nama }} {{ $pangkat->golongan }}</p>
                                    @endforeach
                                </td>
                                <td class="px-6 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap">
                                    {{ $tupeg->tunkins->periode }}
                                </td>
                                <td class="px-6 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap">
                                    @rupiah($tupeg->nominal)
                                </td>

                                <td class="px-6 py-4">
                                    <a href="{{ route('pangkats.edit', $tupeg) }}"
                                       class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">Edit</a>
                                    <form action="{{ route('pangkats.destroy', $tupeg) }}" method="post" class="inline-block">
                                        @csrf
                                        @method('DELETE')
